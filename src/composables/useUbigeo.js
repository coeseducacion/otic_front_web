...
export const useUbigeo = () => {
  ...
  const getDistrictWithRegion = async (params={},rowsPerPage=15,toPage=1) => {
    try {
      loading.value = true
        const response = await api.post(`/districts-with-region/search?page=${toPage}&limit=${rowsPerPage}`, params, {
        headers: {
